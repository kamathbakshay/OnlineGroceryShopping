...
def admin():
    ...
    db.cur.execute("select  phistory.pid,pname,priceperunit,catid, sum(quantity) as totalQuantity,sum(quantity)*priceperunit as totalPrice from product,phistory where product.pid = phistory.pid   group by phistory.pid order by catid,totalQuantity desc,catid ")
